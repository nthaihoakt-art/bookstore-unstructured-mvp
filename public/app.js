...
async function bookDetail(id){ try { const b=await api('/api/books/'+id); const docs = (b.documents || []).map(d => ({ 'Mã': d.id, 'Tên tài liệu': `<a href="#" onclick="docDetail(${d.id}); return false;">${esc(d.original_name)}</a>`, 'Loại': docTypeLabels[d.doc_type] || d.doc_type, 'Tiêu đề': esc(d.title || ''), 'Ngày tạo': d.created_at })); const el = field('detail'); if(el) { el.innerHTML=`<div class="card"><h3>${esc(b.title)}</h3><p><b>Tác giả:</b> ${esc(b.author)} | <b>NXB:</b> ${esc(b.publisher)} | <b>Tồn:</b> ${b.stock_quantity}</p><p>${esc(b.description)}</p><h4>Tài liệu liên quan</h4>${table(docs)}</div>`; el.scrollIntoView({ behavior: 'smooth' }); } } catch (e) { alert('Lỗi hiển thị chi tiết sách: ' + e.message); } }
